'''
Implementation of HRR from https://github.com/MahmudulAlam/Holographic-Reduced-Representations/
'''
...
def normal(shape, seed):
    torch.manual_seed(seed)
    d = float(torch.prod(torch.tensor(shape[1:])))
    return torch.normal(mean=0.0, std=1.0 / (d ** 0.5), size=shape)
